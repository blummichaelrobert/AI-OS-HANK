...
DeadBranch | Any unit — domain-agnostic law | ad-hoc %compose | 2026-09-05 | **THE LAW: Tier 1 validates RECORDS, never REACHABILITY — a declared error branch that no input can trigger is invisible to the validator forever.** THE INSTANCE: a contract declared a `no_structure_found` failure path; the unit's own logic guaranteed at least one node for any non-empty input, so the branch could never fire, and every gate passed on every run without ever revealing it.  An unconditional disambiguator DELETES an error class instead of branching on it — a filename suffix applied on every write, not only on collision, makes a collision failure unreachable and therefore undeclarable. THE BUILD TEST: before declaring a failure branch, ask whether an unconditional construction removes the condition entirely; if it does, that is cheaper than the branch and cheaper than the check that would have guarded it. | Live test, branch unreachable across 7 gates; amendment live-confirmed across 11 gates | [C]
